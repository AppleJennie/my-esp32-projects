...
void HRDataUpdateTask(void *argument)
{
	...
	while(1)
	{
		if(Page_Get_NowPage()->page_obj == &ui_HRPage || SportNow.state != SPORT_IDLE)
		{
			...
			if(!HWInterface.HR_meter.ConnectionError)
			{
				...
				xTaskResumeAll();
				if(HWInterface.HR_meter.HrRate != hr_temp && hr_temp>40 && hr_temp<200)
				{
...
void SensorDataUpdateTask(void *argument)
{
	...
	while(1)
	{
		...
		// sport session data update
		Sport_Update500ms();

		// sedentary and sport remind
		{
			static uint16_t sedentary_last_steps = 0xFFFF;
			static uint16_t sedentary_cnt = 0;   //unit: 500ms
			static uint8_t sport_remind_date = 0;
			HW_DateTimeTypeDef nowdatetime;

			if(HWInterface.IMU.Steps != sedentary_last_steps)
			{
				sedentary_last_steps = HWInterface.IMU.Steps;
				sedentary_cnt = 0;
			}
			else if(sedentary_cnt < 0xFFFF)
			{
				sedentary_cnt++;
			}
			...
			if(SportNow.state == SPORT_IDLE && Page_Get_NowPage()->page_obj != &ui_RemindPage)
			{
				//sedentary: 60min without steps during 8:00-22:00
				if(SportSetting.sedentary_en && sedentary_cnt >= 7200 &&
					nowdatetime.Hours >= 8 && nowdatetime.Hours < 22)
				{
					uint8_t IdleBreakstr2 = 0;
					sedentary_cnt = 0;
					ui_RemindType = REMIND_SEDENTARY;
					osMessageQueuePut(IdleBreak_MessageQueue, &IdleBreakstr2, 0, 1);
					Page_Load(&Page_Remind);
				}
				//sport remind: after the remind time and today sport not reach the goal
				else if(nowdatetime.Date != sport_remind_date &&
					(nowdatetime.Hours > SportSetting.remind_hour ||
					 (nowdatetime.Hours == SportSetting.remind_hour && nowdatetime.Minutes >= SportSetting.remind_min)) &&
					SportTodayMin < SportSetting.sport_goal_min)
				{
					uint8_t IdleBreakstr2 = 0;
					sport_remind_date = nowdatetime.Date;
					ui_RemindType = REMIND_SPORT;
					osMessageQueuePut(IdleBreak_MessageQueue, &IdleBreakstr2, 0, 1);
					Page_Load(&Page_Remind);
				}
			}
