...
private:
    ...
    void InitializeTools() {
        ...
        mcp.AddTool(
            "self.sleep.start",
            "开始睡眠监测会话，启动数据统计和记录。融合引擎（R60雷达+鼾声）持续在后台运行。",
            {},
        ...
        mcp.AddTool(
            "self.sleep.stop",
            "停止睡眠监测会话，结束数据统计。融合引擎继续在后台运行。",
            {},
            [](const PropertyList&) -> ReturnValue {
                SleepDataCenter::GetInstance().StopSession();
                return ReturnValue("{\"success\":true,\"message\":\"已停止睡眠监测\"}");
            }
        );

        mcp.AddTool(
            "self.sleep.status",
            "查询当前睡眠监测状态。只基于返回 JSON 回答，不要编造数据。",
            {},
            [](const PropertyList&) -> ReturnValue {
        ...
        mcp.AddTool(
            ...
            "查询当前生命体征（心率、呼吸率、血氧）。只基于返回 JSON 回答，不要编造数据。如果没有数据，直接告诉用户数据不足。",
            {},
            [](const PropertyList&) -> ReturnValue {
        ...
        mcp.AddTool(
            ...
            "查询鼾声状态。只基于返回 JSON 回答，不要编造数据。如果没有检测到鼾声，只能说'本次监测未检测到明显疑似鼾声'，不能说'一定没打鼾'。",
            {},
            [](const PropertyList&) -> ReturnValue {
        ...
        mcp.AddTool(
            ...
            "查询睡眠环境（温度、湿度、光照、舒适度）。只基于返回 JSON 回答，不要编造数据。",
            {},
            [](const PropertyList&) -> ReturnValue {
        ...
        mcp.AddTool(
            ...
            "查询呼吸暂停疑似风险。只基于返回 JSON 回答，不要编造数据，不要给出医学诊断。只能说'疑似风险'或'数据不足'。必须提示'本结果仅作为家庭睡眠观察参考，不能替代医学诊断'。",
            {},
            [](const PropertyList&) -> ReturnValue {
