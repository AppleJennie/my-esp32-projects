...
#define BL_MIN_DURATION_MS   120000  /* 至少 2 分钟 */
...
void sleep_baseline_feed(const radar_feature_t *r, const audio_feature_t *a)
{
    ...
    bool conditions_met = presence && in_bed && breath_ok && heart_ok && still;

    if (!conditions_met) {
        s_unstable_streak++;
        if (s_unstable_streak >= BL_RESET_STREAK) {
            /* 连续 30 秒不稳定 → 完全重置 */
            if (s_bl_start_ms > 0) {
                ESP_LOGI(TAG, "Baseline RESET after %d sec unstable", s_unstable_streak);
            }
            s_bl_start_ms    = 0;
            s_bl_count       = 0;
            s_unstable_streak = 0;
