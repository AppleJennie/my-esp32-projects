...
{
    sleep_radar_data_lock();
    if (len >= 1) g_radar_data.combined_presence    = payload[0];
    if (len >= 2) g_radar_data.combined_sleep_state  = payload[1];
    if (len >= 3) g_radar_data.avg_heart_rate_10min  = payload[2];
    if (len >= 4) g_radar_data.avg_breath_rate_10min = payload[3];
    if (len >= 5) g_radar_data.turn_over_count_10min = payload[4];
    if (len >= 6) g_radar_data.large_motion_ratio    = payload[5];
    if (len >= 7) g_radar_data.small_motion_ratio    = payload[6];
    if (len >= 8) g_radar_data.apnea_count_10min     = payload[7];
    g_radar_data.last_update_ms = now_ms;
    sleep_radar_data_unlock();
}
