...
const char *sleep_fusion_event_label(int event)
{
    switch (event) {
        ...
        case FUSION_OBSTRUCTIVE_LIKE: return "obstructive_like";        /* 阻塞倾向，非医学诊断 */
        case FUSION_CENTRAL_LIKE:     return "central_like";            /* 中枢倾向，非医学诊断 */
        case FUSION_HYPOPNEA_LIKE:    return "hypopnea_like";           /* 疑似低通气，非医学诊断 */
        case FUSION_RADAR_ONLY_APNEA: return "radar_only_apnea";        /* 纯雷达暂停，无音频确认 */
...
void sleep_fusion_tick(void)
{
    ...
    bool startup_warmup = false;  /* 调试关闭, 正式: uptime_ms < 30000 */
    ...
    if (apnea_signal && wf.motion_max_10s <= 20.0f) {
        ...
        if (is_obstructive) {
            ...
            if (conf >= 60 && state_elapsed >= 10) {
                out.event = FUSION_OBSTRUCTIVE_LIKE;
                if (s_state != FUSION_OBSTRUCTIVE_LIKE) s_apnea_total++;  /* 仅首次计 */
            ...
            if (conf >= 55 && state_elapsed >= 10) {
                out.event = FUSION_CENTRAL_LIKE;
                if (s_state != FUSION_CENTRAL_LIKE) s_apnea_total++;  /* 仅首次计 */
        ...
        out.event       = FUSION_HYPOPNEA_LIKE;
        out.confidence  = (uint8_t)conf;
        out.duration_sec = (uint16_t)state_elapsed;
        if (s_state != FUSION_HYPOPNEA_LIKE) s_hypopnea_total++;  /* 仅首次计 */
        ...
        /* ── 恢复呼吸检测 ── */
        bool was_abnormal = (s_state == FUSION_OBSTRUCTIVE_LIKE ||
                             s_state == FUSION_CENTRAL_LIKE ||
                             s_state == FUSION_HYPOPNEA_LIKE);
...
emit:
    /* ── 证据标记 ── */
    out.evidence_flags = 0;
    if (wf.breath_amp_drop_ratio > 0.3f) out.evidence_flags |= (1 << 0);  /* RADAR_DROP */
    if (wf.motion_max_10s < 15.0f)    out.evidence_flags |= (1 << 1);  /* LOW_MOTION */
    if (af_ok && s_af.is_snoring)     out.evidence_flags |= (1 << 2);  /* SNORE_ACTIVE */
    if (af_ok && !s_af.is_snoring)    out.evidence_flags |= (1 << 3);  /* AUDIO_QUIET */
    if (out.baseline_valid)           out.evidence_flags |= (1 << 6);  /* BASELINE_OK */
    if (wf.recovery_breath_detected)  out.evidence_flags |= (1 << 7);  /* RECOVERY_BREATH */
