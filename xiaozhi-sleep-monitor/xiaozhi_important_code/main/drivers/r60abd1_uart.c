                            ...
                            uint16_t len, const uint8_t *p, uint32_t now)
{
    ...
    if (ctrl == 0x84) {
        switch (cmd) {
            case 0x01: case 0x81:  /* 在床/离床 */
                if (len >= 1) sleep_radar_data_update_in_bed(p[0], now);
                return;
            case 0x02:             /* 睡眠分期 */
                if (len >= 1) sleep_radar_data_update_sleep_stage(p[0], now);
                return;
            case 0x03:             /* 清醒时长 (2 bytes, minutes) */
                if (len >= 2) {
                    uint16_t v = ((uint16_t)p[0] << 8) | p[1];
                    sleep_radar_data_update_awake_duration(v, now);
                }
                return;
            case 0x04:             /* 浅睡时长 (2 bytes, minutes) */
                if (len >= 2) {
                    uint16_t v = ((uint16_t)p[0] << 8) | p[1];
                    sleep_radar_data_update_light_sleep_duration(v, now);
                }
                return;
            case 0x05:             /* 深睡时长 (2 bytes, minutes) */
                if (len >= 2) {
                    uint16_t v = ((uint16_t)p[0] << 8) | p[1];
                    sleep_radar_data_update_deep_sleep_duration(v, now);
                }
                return;
            case 0x06:             /* 睡眠评分 */
                if (len >= 1) sleep_radar_data_update_sleep_score(p[0], now);
                return;
            case 0x07:             /* 睡眠质量 */
                if (len >= 1) sleep_radar_data_update_sleep_quality(p[0], now);
                return;
            case 0x08:             /* 异常挣扎 */
                if (len >= 1) sleep_radar_data_update_struggle(p[0], now);
                return;
            case 0x09:             /* 无人计时 (2 bytes) */
                if (len >= 2) {
                    uint16_t v = ((uint16_t)p[0] << 8) | p[1];
                    sleep_radar_data_update_no_person_timer(v, now);
                }
                return;
            case 0x0C: case 0x8C:  /* 综合状态 (composite: presence/sleep/hr/br/turnover/motion/apnea) */
                sleep_radar_data_update_sleep_composite(p, (uint8_t)len, now);
                return;
            default: return;
        }
    }
...
static int try_extract_frame(void)
{
    ...
    /* 收到任何有效帧都标记雷达已连接 */
    {
        ...
        sleep_radar_data_t *rd = sleep_radar_data_get();
        if (rd && !rd->radar_connected) {
            rd->radar_connected = true;
        }
...
void r60abd1_send_init_commands(void)
{
    ...
    const uint8_t c4[] = R60_INIT_SLEEP_MODE;  r60abd1_uart_send(c4, 10); vTaskDelay(pdMS_TO_TICKS(200));
    ESP_LOGI(TAG, "Init done: presence+breath+heart+sleep_mode enabled");
}
...
void r60abd1_query_task(void *pvParameters)
{
    ...
    const uint8_t q_sc[] = R60_QUERY_SLEEP_COMPOSITE;

    uint32_t last_1s = 0, last_3s = 0, last_10s = 0;
    ...
    while (s_query_running) {
        ...
        /* 每 10 秒：综合睡眠状态（变化慢，不必高频查询） */
        if (now - last_10s >= 10000) {
            last_10s = now;
            r60abd1_uart_send(q_sc, 10);
        }
