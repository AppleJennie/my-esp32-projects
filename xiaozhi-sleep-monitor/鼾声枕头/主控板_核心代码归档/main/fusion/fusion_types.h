...
#define FUSION_RING_SEC  60    /* 60秒够用, 省SRAM (~9KB) */
...
typedef struct {
    ...
    bool     breath_wave_valid;     uint8_t breath_wave[5]; float breath_amp;  float breath_quality; float breath_cv;
...
typedef enum {
    ...
    FUSION_OBSTRUCTIVE_LIKE         = 2,  /* 阻塞倾向（有鼾声，但无气流/胸腹数据确认） */
    FUSION_CENTRAL_LIKE             = 3,  /* 中枢倾向（安静，但无气流/胸腹数据确认） */
    FUSION_HYPOPNEA_LIKE            = 4,  /* 疑似低通气 */
    ...
    FUSION_RADAR_ONLY_APNEA         = 10, /* 雷达疑似暂停（无音频辅助） */
} fusion_event_t;

/* 事件证据位（可解释融合） */
#define EVIDENCE_RADAR_DROP       (1 << 0)  /* 呼吸幅度下降 */
#define EVIDENCE_LOW_MOTION       (1 << 1)  /* 体动伪影低 */
#define EVIDENCE_SNORE_ACTIVE     (1 << 2)  /* 鼾声活跃 */
#define EVIDENCE_AUDIO_QUIET      (1 << 3)  /* 音频安静(中枢特征) */
#define EVIDENCE_SPO2_DROP        (1 << 4)  /* 伴随血氧下降 */
#define EVIDENCE_HR_REBOUND       (1 << 5)  /* 心率反跳 */
#define EVIDENCE_BASELINE_OK      (1 << 6)  /* 基线就绪 */
#define EVIDENCE_RECOVERY_BREATH  (1 << 7)  /* 恢复呼吸 */
