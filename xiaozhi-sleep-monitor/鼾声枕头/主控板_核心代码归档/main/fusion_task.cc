...
/* BLE: 暂时禁用，Windows 命令行长度限制，改用外置 BLE 模块
...
static void read_onboard_sensors(void)
{
    ...
    if (ap3216c_read(&als) == ESP_OK) {
        light = (int)als.als;
        /* SHT30 失败时 temp/humi 保持 -999，调用方知道无效 */
    }
#endif

    if (has_env) {
        SleepDataCenter::GetInstance().UpdateEnvironment(temp, humi, light);
    }
}
...
static void fusion_tick_task(void *pvParameters)
{
    ...
    while (s_running) {
        ...
        if (sleep_radar_data_get_snapshot(&radar_snapshot)) {
            if (sleep_radar_data_is_fresh(radar_snapshot.last_update_ms, RADAR_STALE_MS)) {
                ...
                if (rf.breath_wave_valid && rf.breath_amp > 0) {
                    SleepDataCenter::GetInstance().UpdateBreathWave(
                        rf.breath_amp, now_ms);
                }

                /* ★ 波形深度特征：breath_cv 来自 adapter 实时计算 */
                SleepDataCenter::GetInstance().UpdateWaveFeatures(
                    rf.breath_cv,           /* adapter 峰值检测 */
                    0.0f,                    /* breath_pause_sec */
                    rf.breath_quality,       /* adapter 质量 */
                    0.0f,                    /* hr_sdnn */
                    0.0f);                   /* hrv_quality */

                /* ★ 雷达 composite 数据：apnea + 翻身 */
                SleepDataCenter::GetInstance().UpdateRadarComposite(
                    radar_snapshot.apnea_count_10min,
                    radar_snapshot.turn_over_count_10min);
        ...
        if (sleep_fusion_get_result(&fr)) {
            if (fr.data_valid) {
                /* 鼾声事件（不重复调 UpdateRadar，雷达数据已在步骤2喂入） */
                if (fr.snoring) {
        ...
        /* ====== 6b. 更新呼吸专项评分缓存 ====== */
        SleepDataCenter::GetInstance().UpdateCachedSnoreRespScore();
...
bool fusion_task_init(void)
{
    ...
    /* BLE: 暂时禁用，改用外置 BLE 模块
    watch_ble_receiver_init();
    */
