...
static int fusion_event_to_id(fusion_event_t e)
{
    switch (e) {
        ...
        case FUSION_HYPOPNEA_LIKE:         return 2;
        case FUSION_OBSTRUCTIVE_LIKE:      return 3;
        case FUSION_CENTRAL_LIKE:          return 3;
        case FUSION_RADAR_ONLY_APNEA:      return 3;
...
void main_report_uart_send(void)
{
    ...
    if (has_radar_snapshot && radar_status >= 1) {
        ...
        heart_valid = (radar_snapshot.heart_rate > 0 &&
                       (uint32_t)(now_tick_ms - radar_snapshot.last_heart_rate_ms) <= 10000U) ? 1 : 0;
        breath_bpm = radar_snapshot.breath_rate;
        breath_valid = (radar_snapshot.breath_rate > 0 &&
                        (uint32_t)(now_tick_ms - radar_snapshot.last_breath_rate_ms) <= 10000U) ? 1 : 0;
        motion_valid = (radar_snapshot.last_body_motion_ms != 0 &&
                        (uint32_t)(now_tick_ms - radar_snapshot.last_body_motion_ms) <= 10000U) ? 1 : 0;
    ...
    /* TODO: 接真实 WiFi/SD 状态 */
    int wifi_ok = 2;  /* 2=未接入/未知, 0=离线, 1=正常 */
    int sd_ok = 2;    /* 2=未接入/未知, 0=离线, 1=正常 */
...
#if CONFIG_ENABLE_SHT30
    {
        ...
        if (sht30_read(&sht) == ESP_OK) {
            temp_valid = 1;
            temp_x10 = (int)(sht.temperature * 10.0f);
            hum_x10  = (int)(sht.humidity * 10.0f);
        }
    }
#endif

    /* ── 新增6字段 ── */
    /* breath_amp: 从雷达波形快照计算呼吸幅度 */
    int breath_amp_u8 = 0;
    if (has_radar_snapshot && radar_status >= 1 && breath_valid) {
        radar_feature_t rf;
        r60abd1_adapter_convert(&radar_snapshot, &rf);
        if (rf.breath_wave_valid && rf.breath_amp > 0) {
            int amp = (int)(rf.breath_amp + 0.5f);
            breath_amp_u8 = (amp > 255) ? 255 : ((amp < 0) ? 0 : amp);
        }
    }

    /* baseline */
    int baseline_ok = sleep_baseline_is_ready() ? 1 : 0;
    int baseline_progress = sleep_baseline_progress_sec();
    if (baseline_progress > 100) baseline_progress = 100;
    if (baseline_progress < 0)   baseline_progress = 0;

    /* event_confidence */
    int event_confidence = 0;
    if (sleep_fusion_get_result(&fr) && fr.data_valid) {
        event_confidence = (int)fr.confidence;
        if (event_confidence > 100) event_confidence = 100;
    } else {
        memset(&fr, 0, sizeof(fr));
    }
    ...
    if (has_radar_snapshot && radar_status >= 1) {
        uint32_t now_tick_ms = xTaskGetTickCount() * portTICK_PERIOD_MS;
        if (radar_snapshot.last_distance_ms != 0 &&
            (uint32_t)(now_tick_ms - radar_snapshot.last_distance_ms) <= 10000U) {
            distance_cm = (int)radar_snapshot.distance_cm;
        }
    }

    /* snore_resp_score */
    int snore_resp_score = SleepDataCenter::GetInstance().GetCachedSnoreRespScore();

    int len = snprintf(buf, sizeof(buf),
        "MAIN,%lu,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%u,%u,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
    ...
    if ((uint32_t)(now_ms - s_last_stat_ms) >= 5000U) {
        ESP_LOGI(TAG, "MAIN_TX count=%lu radar=%d hr=%d/%d br=%d/%d risk=%d/%d | ext: amp=%d bl=%d/%d conf=%d dist=%d resp_score=%d",
                 (unsigned long)s_send_count, radar_status,
                 heart_valid, heart_bpm, breath_valid, breath_bpm,
                 risk_valid, risk_level,
                 breath_amp_u8, baseline_ok, baseline_progress,
                 event_confidence, distance_cm, snore_resp_score);
