...
static void EnvPage_timer_cb(lv_timer_t * timer)
{
    ...
    const char * posture_str = "未知";
    switch(HWInterface.IMU.posture)
    {
        case POSTURE_SUPINE: posture_str = "仰卧"; break;
        case POSTURE_LEFT:   posture_str = "左侧卧"; break;
        case POSTURE_RIGHT:  posture_str = "右侧卧"; break;
        case POSTURE_PRONE:  posture_str = "俯卧"; break;
        default:             posture_str = "未知"; break;
    }
    lv_label_set_text(ui_PostureLabel, posture_str);
}
...
void ui_EnvPage_screen_init(void)
{
    ...
    ui_PostureLabel = lv_label_create(ui_EnvPage);
    lv_obj_set_width(ui_PostureLabel, LV_SIZE_CONTENT);
    lv_obj_set_height(ui_PostureLabel, LV_SIZE_CONTENT);
    lv_obj_set_x(ui_PostureLabel, 0);
    lv_obj_set_y(ui_PostureLabel, 140);
    lv_obj_set_align(ui_PostureLabel, LV_ALIGN_CENTER);
    lv_label_set_text(ui_PostureLabel, "未知");
    lv_obj_set_style_text_color(ui_PostureLabel, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_PostureLabel, &ui_font_Cuyuan20, LV_PART_MAIN | LV_STATE_DEFAULT);
