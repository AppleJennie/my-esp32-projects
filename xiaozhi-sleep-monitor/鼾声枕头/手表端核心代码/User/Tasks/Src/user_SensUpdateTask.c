...
void MPUCheckTask(void *argument)
{
	while(1)
	{
		/* Skip wrist check during sleep monitoring to avoid waking screen */
...
void HRDataUpdateTask(void *argument)
{
	...
	while(1)
	{
		/* Sleep monitoring takes over HR sensor, skip here */
		if(SleepMonitor_IsActive())
		{
			osDelay(50);
			continue;
	...
	while(1)
	{
		...
		if(osMessageQueueGet(HomeUpdata_MessageQueue, &HomeUpdataStr, NULL, 0)==osOK)
		{
			...
			if(!(HWInterface.IMU.ConnectionError))
			{
				...
				HWInterface.IMU.posture = MPU_Get_SleepPosture();
			}
		...
		if(Page_Get_NowPage()->page_obj == &ui_SPO2Page)
		{
			...
			if(!HWInterface.HR_meter.ConnectionError)
			{
				...
				uint8_t spo2 = SPO2_Estimate(ppg, hr);
				if (spo2 > 0) {
					HWInterface.HR_meter.SPO2 = spo2;
				}
			}
		}
		// Env Page
